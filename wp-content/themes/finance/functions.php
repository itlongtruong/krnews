...
function htmlwp_pagination()
{
	...
	if (is_array($pages)) {
		...
		echo ' <div class="pagination--wrapper clearfix bdtop--1 bd--color-2 ptop--60 pbottom--30"> <ul class="pagination float--right">';
...
function ah_remove_custom_post_type_slug($post_link, $post, $leavename)
{
	...
	if (!in_array($post->post_type, array('danh-muc')) || 'publish' != $post->post_status)
		return $post_link;
...
function ah_parse_request_tricksy($query)
{
	...
	if (!empty($query->query['name']))
		$query->set('post_type', array('post', 'danh-muc', 'page'));
}
